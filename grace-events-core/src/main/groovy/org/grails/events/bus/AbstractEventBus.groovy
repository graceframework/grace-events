...
abstract class AbstractEventBus implements EventBus {
    ...
    protected final Map<CharSequence, Collection<Subscription>> subscriptions = new ConcurrentHashMap<CharSequence, Collection<Subscription>>()
            .withDefault {
    ...
    Subjects unsubscribeAll(CharSequence event) {
        Collection<Subscription> subs = subscriptions.get(event.toString())
        for (sub in subs) {
            if (!sub.isCancelled()) {
                sub.cancel()
            }
        ...
        if (TransactionSynchronizationManager.isSynchronizationActive()) {
            ...
            if (subscriptions.containsKey(eventId)) {
                ...
                if (!eventSubscriptions.isEmpty()) {
                    buildNotificationTrigger(event, eventSubscriptions).run()
                }
            }
        }
        return this
    ...
    EventEmitter sendAndReceive(Event event, Closure reply) {
        ...
        if (reply == null) {
            throw new IllegalArgumentException('Argument [reply] cannot be null')
        }
        String eventId = event.id
        if (subscriptions.containsKey(eventId)) {
